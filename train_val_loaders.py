...
class _MusicDataset(Dataset):
    ...
    def __init__(self, directory):
        ...
        assert self.dir[-4:] == 'data', 'Input argument must end with "data".'
        assert os.path.isdir(self.dir) == True, 'Input argument must be a valid directory.'
        assert os.path.isdir(self.dir[:-4]+'labels') == True, 'There must be a corresponding "labels" directory.'

        self.list_of_data = [i if i[-3:] == 'npy' for i in os.listdir(directory)]
    ...
    def __getitem__(self, index):
        ...
        # Get data sample
        data_sample = np.load(os.path.join(self.dir,self.list_of_data[index]))
        # Load data label
        midi = pretty_midi.PrettyMIDI(os.path.join(self.dir[:-4]+'labels',
                                                   self.list_of_data[index][:-4]+'mid'))
        data_label = midi.get_piano_roll()

        return data_sample, data_label

def get_data_loaders(train_path, val_path, test_path, batch_size=1,
                     shuffle=True):
