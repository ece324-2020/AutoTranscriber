...
def qualitative_result(net, preds, mean, stdDev, rand=True):
    ...
    label_sample = label_sample[None, :, :]
    print(label_sample.shape) 
    ...
    #index = torch.tensor([0, 126])
    #input_tensor = torch.index_select(input_tensor, , index)
    input_tensor = input_tensor[:, :, :, 0:126]
    input_tensor = (input_tensor - mean)/stdDev
    print(input_tensor.shape) 
    device = next(net.parameters()).device
    prediction, _ = net(input_tensor.to(device))
    print(prediction)
    ...
    plt.imshow(label_sample[0], cmap="Blues", aspect="auto", origin='lower') # auto makes each box rectangular
    plt.colorbar()
    plt.subplot(212)
